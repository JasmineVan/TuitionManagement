	...
	.then(json => {
		...
		if(json.code === 0){
			...
			if(json.data.token === 0){
				...
				//window.location.href = '/FE/user/home.html'
			}
			else{
				if(json.data.status === 'đã vô hiệu hóa'){
					...
					console.log("Đăng Nhập Thành Công ELSE")
					localStorage.setItem("account", JSON.stringify(json.data));
					window.location.href = '../FE/user/home.html'
				}
			}	
		}
		else if(json.code === 1){
			localStorage.setItem("accountAdmin", JSON.stringify(json.data));
			window.location.href = '../FE/admin/home.html'
		}
		else{
...
//Transfer Money
function validateChuyenTien(mssv, money, otp){

	if(mssv === ''){
		showErrorToast('Vui lòng nhập mã số sinh viên')
		return false
	}
	if(money === ''){
...
if (tranfer_money) {
    ...
    btn_sendotp.addEventListener("click", (e) => {
        ...
        const mssv_transfer = document.getElementById('mssv_transfer').value;
        if (mssv_transfer === "") {
            showErrorToast("Phone không được rỗng")
            return;
        }

		fetch('http://localhost:3000/user/otp', {
			headers:{
				'Content-Type':'application/x-www-form-urlencoded'
			}, 
			method: 'POST',
			body:'mssv=' + mssv_transfer + '&type=' + 'transfer_money' + '&account=' + account,
		})
		.then(res => res.json())
		.then(json => {
			console.log(json)
...
$('#mssv_transfer').change(e => {
	console.log("change")
	let mssv = $("#mssv_transfer").val()
	console.log(mssv)
	fetch('http://localhost:3000/user/getMoneyByMssv', {
		headers:{
			'Content-Type':'application/x-www-form-urlencoded'
		}, 
		method: 'POST',
		body:'mssv=' + mssv + '&account=' + JSON.stringify(account),
	})
	.then(res => res.json())
	.then(json => {
		console.log(json)
		if(json.code === 0){
			...
			$('#money_transfer').val(json.data.tuition)
		}
		else{
			showErrorToast(json.message)
		}
...
$('#btn_transfer').click(e => {
	...
	let receiver = $('#receiver_transfer').val()
	let mssv = $('#mssv_transfer').val()
	let money = $('#money_transfer').val()
	let otp = $('#otp_transfer').val()
	let account = localStorage.getItem("account");

	if(!validateChuyenTien(mssv, money, otp)){
		return 
	}

	fetch('http://localhost:3000/user/chuyentien', {
		headers:{
			'Content-Type':'application/x-www-form-urlencoded'
		}, 
		method: 'POST',
		body:'receiver=' + receiver + '&mssv=' + mssv + '&money=' + money + '&otp_code=' + otp + '&accountLocalStorage=' + account,
	})
	.then(res => res.json())
	.then(json => {
		console.log(json)
		if(json.code === 0){
			...
			setTimeout(() => {
				window.location.href = "../user/tuition.html"
			}, 2000)
		}
		else{
			showErrorToast(json.message)
